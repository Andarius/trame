...
export type ViewConfig = { sorts: Sort[]; filters: Filter[]; groupBy?: string | null; aggs?: Record<string, Agg | null> };
...
const NO_VALUE = new Set<FilterOp>(["empty", "not_empty", "checked", "unchecked"]);
const catOf = (p: UdbProp): Cat => CAT[p.type] ?? "text";
const opsFor = (p: UdbProp): readonly FilterOp[] => OPS[catOf(p)];

// value extraction

const dateStart = (v: unknown): string => (typeof v === "object" && v ? (v as { start?: string }).start ?? "" : String(v ?? ""));
const textOf = (p: UdbProp, r: UdbRow): string =>
  p.type === "formula" || p.type === "rollup" ? String(r.derived[p.id] ?? "") : String(r.vals[p.id] ?? "");
const numOf = (p: UdbProp, r: UdbRow): number | null => {
  const raw = p.type === "formula" || p.type === "rollup" ? r.derived[p.id] : r.vals[p.id];
...
function sortValue(p: UdbProp, r: UdbRow): number | string | null {
  ...
  if (p.type === "relation") {
    return (r.relations[p.id] ?? []).map((c) => c.title).join(", ").toLowerCase();
  }
  const v = r.vals[p.id];
  switch (p.type) {
    case "number":
...
function passes(p: UdbProp, r: UdbRow, f: Filter): boolean {
  ...
  if (p.type === "relation") {
    ...
    const hit = (r.relations[p.id] ?? []).some((c) => c.title.toLowerCase().includes(needle));
  ...
  switch (f.op) {
    ...
    case "is_not": return text !== needle;
  }
  return true;
}
...
export type Group = { key: string; label: string; color?: string; rows: UdbRow[] };
...
export function groupRows(rows: UdbRow[], p: UdbProp): Group[] {
  ...
  const add = (key: string, label: string, color: string | undefined, r: UdbRow) => {
    let g = map.get(key);
  ...
  for (const r of rows) {
    ...
    if (p.type === "select") {
      ...
      const label = ((r.vals[p.id] as string[]) ?? []).map((id) => opts.find((x) => x.id === id)?.name ?? id).join(", ");
  ...
  if (p.type === "select") {
    ...
    rest.sort((a, b) => (order.get(a.key) ?? opts.length) - (order.get(b.key) ?? opts.length));
...
export function aggregate(rows: UdbRow[], p: UdbProp, agg: Agg): number | null {
  const nums = rows.map((r) => numOf(p, r)).filter((n): n is number => n !== null);
...
export const fmtAgg = (n: number): string => (Number.isInteger(n) ? String(n) : n.toFixed(2));

// Apply filters (AND) then multi-sort. Returns a new array; input order preserved for ties.
export function applyView(rows: UdbRow[], props: UdbProp[], view: ViewConfig): UdbRow[] {
  const byId = new Map(props.map((p) => [p.id, p]));
  const active = view.filters.filter((f) => byId.has(f.propId));
  let out = active.length ? rows.filter((r) => active.every((f) => passes(byId.get(f.propId)!, r, f))) : rows;
...
const isEmptyConfig = (c: ViewConfig) => !c.sorts.length && !c.filters.length && !c.groupBy;
export const newTab = (name: string, config: ViewConfig = emptyConfig()): ViewTab => ({
...
function legacyView(dbId: string): ViewConfig {
  try {
    const v = JSON.parse(localStorage.getItem(`trame:udbview:${dbId}`) ?? "null");
    if (v && Array.isArray(v.sorts) && Array.isArray(v.filters)) return v;
    const old = JSON.parse(localStorage.getItem(`trame:udbsort:${dbId}`) ?? "null");
    if (old && typeof old.propId === "string") return { sorts: [old], filters: [] };
  } catch { /* fall through */ }
  return emptyConfig();
}
export function loadTabs(dbId: string): ViewTabs {
  try {
    const v = JSON.parse(localStorage.getItem(key(dbId)) ?? "null");
    if (v && Array.isArray(v.tabs)) {
      const tabs = (v.tabs as ViewTab[]).filter((t) => t && typeof t.id === "string" && t.config);
      if (tabs.length) return { tabs, active: tabs.some((t) => t.id === v.active) ? v.active : tabs[0].id };
    }
  } catch { /* fall through */ }
...
export const saveTabs = (dbId: string, v: ViewTabs) => {
  if (v.tabs.length === 1 && v.tabs[0].name === "Table" && isEmptyConfig(v.tabs[0].config)) {
    localStorage.removeItem(key(dbId));
...
export function ViewTabsBar({ state, onChange }: { state: ViewTabs; onChange: (v: ViewTabs) => void }) {
  const [renaming, setRenaming] = useState<string | null>(null);
  const [draft, setDraft] = useState("");

  const rename = (id: string) => {
    const name = draft.trim();
    if (name) onChange({ ...state, tabs: state.tabs.map((t) => (t.id === id ? { ...t, name } : t)) });
    setRenaming(null);
  };
  const add = () => {
  ...
  return (
    <div className="flex items-center gap-0.5">
        ...
        renaming === t.id
          ? (
            <input
              key={t.id}
              autoFocus
              className="w-[90px] rounded-md border border-copper/60 bg-transparent px-1.5 py-0.5 text-[11.5px] text-ink outline-none"
              value={draft}
          ...
          : (
            <div key={t.id} className="group/tab flex items-center">
              <button type="button"
                className={`rounded-md px-1.5 py-0.5 text-[11.5px] transition-colors ${
                  t.id === state.active ? "bg-panel font-medium text-ink" : "text-ink-muted hover:text-ink-soft"
                }`}
                title="double-click to rename"
                onClick={() => onChange({ ...state, active: t.id })}
                onDoubleClick={() => {
                  setRenaming(t.id);
                  setDraft(t.name);
                }}
              ...
              {state.tabs.length > 1 && (
                <button type="button"
                  className="-ml-0.5 px-0.5 text-[9px] text-ink-muted opacity-0 transition-opacity hover:text-blocked group-hover/tab:opacity-100"
      ...
      <button type="button"
        className="px-1.5 py-0.5 text-[12px] text-ink-muted/60 transition-colors hover:text-ink-soft"
        title="new view tab"
        onClick={add}
      >
        ＋
...
const chip = (active: boolean) =>
  `flex items-center gap-1.5 rounded-md border px-2 py-1 text-[11.5px] transition-colors ${
    active ? "border-copper/50 text-copper" : "border-line text-ink-muted hover:text-ink-soft"
  }`;
const propOpts = (props: UdbProp[]) =>
  props.map((p) => ({ value: p.id, label: `${TYPE_GLYPH[p.type] ?? "?"}  ${p.name}` }));

// value editor for one filter rule (option dropdown / date / text), or nothing
function FilterValue({ prop, filter, onChange }: { prop: UdbProp; filter: Filter; onChange: (v: string) => void }) {
  if (NO_VALUE.has(filter.op)) return null;
  const cls = "w-full rounded-md border border-chipline bg-transparent px-2 py-1 text-[11px] text-ink outline-none focus:border-copper/60";
  if (prop.type === "select" || prop.type === "multi_select") {
    ...
    return (
      <Select
        value={filter.value ?? ""}
        placeholder="choose…"
        options={opts.map((o) => ({ value: o.id, label: o.name, dot: o.color }))}
        onChange={onChange}
      />
    );
  }
  if (prop.type === "date") {
    return <input type="date" className={cls} value={filter.value ?? ""} onChange={(e) => onChange(e.target.value)} />;
  }
  return (
    <input
      className={cls}
...
export function ViewToolbar(
  { props, view, onChange }: { props: UdbProp[]; view: ViewConfig; onChange: (v: ViewConfig) => void },
) {
  ...
  const addSort = () => {
    ...
    if (p) onChange({ ...view, sorts: [...view.sorts, { propId: p.id, dir: 1 }] });
  };
  const addFilter = () => {
    const p = props[0];
    if (p) onChange({ ...view, filters: [...view.filters, { propId: p.id, op: opsFor(p)[0], value: "" }] });
  };

  return (
    <div className="flex items-center gap-2">
      <div className="relative">
        <button type="button" className={chip(view.filters.length > 0)} onClick={() => setOpen((o) => (o === "filter" ? null : "filter"))}>
          ▽ Filter{view.filters.length > 0 && ` · ${view.filters.length}`}
        </button>
        {open === "filter" && (
          <Popover onClose={() => setOpen(null)} className="w-[420px] max-w-[92vw] p-2">
            {view.filters.length === 0 && (
              <p className="px-1 py-1.5 text-[11px] text-ink-muted/70">No filters — rows matching every rule are shown.</p>
            )}
            <div className="flex flex-col gap-1.5">
              {view.filters.map((f, i) => {
                const p = byId.get(f.propId) ?? props[0];
                const set = (patch: Partial<Filter>) =>
                  onChange({ ...view, filters: view.filters.map((x, j) => (j === i ? { ...x, ...patch } : x)) });
                return (
                  ...
                  <div key={i} className="flex flex-wrap items-center gap-1">
                    <span className="w-9 shrink-0 text-[10px] text-ink-muted/60">{i === 0 ? "Where" : "and"}</span>
                    ...
                    <div className="min-w-[84px] flex-1">
                      <Select
                        value={f.op}
                        options={opsFor(p).map((o) => ({ value: o, label: OP_LABEL[o] }))}
                        onChange={(v) => set({ op: v as FilterOp })}
                      />
                    </div>
                    {!NO_VALUE.has(f.op) && (
                      <div className="min-w-[80px] flex-1">
                        <FilterValue prop={p} filter={f} onChange={(v) => set({ value: v })} />
                      </div>
                    )}
                    <button type="button"
                      className="shrink-0 px-1 text-ink-muted hover:text-blocked"
                      title="remove"
                      onClick={() => onChange({ ...view, filters: view.filters.filter((_, j) => j !== i) })}
                    >
                      ✕
                    </button>
                  </div>
            ...
            <div className="mt-1.5 flex items-center gap-3 border-t border-line-soft pt-1.5">
              <button type="button" className="text-[11px] text-ink-muted hover:text-copper" onClick={addFilter}>＋ Add filter</button>
              {view.filters.length > 0 && (
                <button type="button" className="text-[11px] text-ink-muted hover:text-blocked" onClick={() => onChange({ ...view, filters: [] })}>
                  Clear all
                </button>
              )}
            </div>
      ...
      <div className="relative">
        <button type="button" className={chip(view.sorts.length > 0)} onClick={() => setOpen((o) => (o === "sort" ? null : "sort"))}>
          ⇅ Sort{view.sorts.length > 0 && ` · ${view.sorts.length}`}
        </button>
        {open === "sort" && (
          <Popover onClose={() => setOpen(null)} className="w-[300px] max-w-[92vw] p-2">
            {view.sorts.length === 0 && <p className="px-1 py-1.5 text-[11px] text-ink-muted/70">No sorts — rows keep their manual order.</p>}
            <div className="flex flex-col gap-1.5">
              {view.sorts.map((s, i) => {
                const move = (dir: -1 | 1) => {
                  const j = i + dir;
                ...
                return (
                  <div key={i} className="flex items-center gap-1">
                    <div className="flex flex-col leading-none">
                      <button type="button" disabled={i === 0} className="text-[7px] text-ink-muted hover:text-ink disabled:opacity-25" onClick={() => move(-1)}>▲</button>
                      <button type="button" disabled={i === view.sorts.length - 1} className="text-[7px] text-ink-muted hover:text-ink disabled:opacity-25" onClick={() => move(1)}>▼</button>
                    </div>
                    <div className="min-w-0 flex-1">
                      <Select
                        value={s.propId}
                        options={propOpts(props)}
                        onChange={(v) => onChange({ ...view, sorts: view.sorts.map((x, j) => (j === i ? { ...x, propId: v } : x)) })}
                      />
                    </div>
                    <div className="flex shrink-0 rounded-md bg-panel p-[2px]">
                      {([[1, "↑ Asc"], [-1, "↓ Desc"]] as const).map(([d, lbl]) => (
                        <button type="button"
                          key={d}
                          className={`rounded px-1.5 py-0.5 text-[10px] ${s.dir === d ? "bg-[#272b37] text-ink" : "text-ink-muted hover:text-ink-soft"}`}
                          onClick={() => onChange({ ...view, sorts: view.sorts.map((x, j) => (j === i ? { ...x, dir: d } : x)) })}
                        >
                          {lbl}
                        </button>
                      ))}
                    ...
                    <button type="button"
                      className="shrink-0 px-1 text-ink-muted hover:text-blocked"
                      title="remove"
                      onClick={() => onChange({ ...view, sorts: view.sorts.filter((_, j) => j !== i) })}
                    >
                      ✕
                    </button>
                  </div>
            ...
            <div className="mt-1.5 flex items-center gap-3 border-t border-line-soft pt-1.5">
              <button type="button" className="text-[11px] text-ink-muted hover:text-copper" onClick={addSort}>＋ Add sort</button>
              {view.sorts.length > 0 && (
                <button type="button" className="text-[11px] text-ink-muted hover:text-blocked" onClick={() => onChange({ ...view, sorts: [] })}>
                  Clear all
                </button>
              )}
            </div>
      ...
      <div className="relative">
        <button type="button" className={chip(!!groupProp)} onClick={() => setOpen((o) => (o === "group" ? null : "group"))}>
          ▤ Group{groupProp && ` · ${groupProp.name}`}
        </button>
        {open === "group" && (
          <Popover onClose={() => setOpen(null)} className="w-[300px] max-w-[92vw] p-2">
            <div className="flex items-center gap-1">
              <span className="w-14 shrink-0 text-[10px] text-ink-muted/60">Group by</span>
              <div className="min-w-0 flex-1">
                <Select
                  value={view.groupBy ?? ""}
                  placeholder="none"
                  options={[{ value: "", label: "— none —" }, ...propOpts(props)]}
            ...
            {groupProp && aggProps.length > 0 && (
              <div className="mt-1.5 border-t border-line-soft pt-1.5">
                <p className="px-1 pb-1 text-[10px] text-ink-muted/60">Aggregates shown on each group (count is always shown)</p>
                    ...
                    <div key={p.id} className="flex items-center gap-1">
                      ...
                      <div className="w-[112px] shrink-0">
                        <Select
                          ...
                          options={[
                            { value: "", label: "— none —" },
                            ...(["sum", "avg", "min", "max"] as const).map((a) => ({ value: a, label: a })),
                          ]}
                          onChange={(v) => onChange({ ...view, aggs: { ...view.aggs, [p.id]: (v || null) as Agg | null } })}
                        />
                      </div>
                    </div>
                  ))}
