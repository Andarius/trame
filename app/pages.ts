...
export async function getPage(id: string) {
  const pg = await db();
  const page = (await pg.query(`select * from pages where id=$1 and not deleted`, [id])).rows[0];
  if (!page) return null;
  ...
  return { ...page, children, databases, sessions };
}

async function endKey(pg: Awaited<ReturnType<typeof db>>, parentId: string | null): Promise<string> {
...
export async function createPage(
  p: { title?: string; parent_id?: string | null; kind?: string; icon?: string | null; client_id?: string | null },
): Promise<string> {
  const pg = await db();
...
export async function movePage(
  ...
  const pg = await db();
  const cur = (await pg.query(`select parent_id from pages where id=$1 and not deleted`, [id])).rows[0] as
...
export async function attachUdbToPage(dbId: string, pageId: string | null): Promise<void> {
  const pg = await db();
  await pg.query(
