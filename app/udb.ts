...
export async function updateUdb(
  ...
  await pg.query(
    `update udb_databases set name = coalesce($2, name),
            ...
            views = case when $6 then $7::jsonb else views end,
...
export async function createProperty(
  ...
  if (p.type === "rollup") {
    ...
    if (!rel || rel.type !== "relation") {
      throw new Error(
        "rollup: relation_prop must be a relation property of this database",
      );
    }
    if (config.agg !== "count" && !config.target_prop) {
      throw new Error("rollup: target_prop required");
    }
  }
...
export async function deleteProperty(id: string): Promise<void> {
  ...
  if (!cur) return;
  const ids = [id];
  if (cur.type === "relation" && cur.config.pair) {
    ids.push(String(cur.config.pair));
  }
  const ownerId = cur.type === "relation"
    ? (cur.config.owner ? id : String(cur.config.pair))
    : null;
  if (ownerId) {
    await pg.query(
...
function rewriteFormula(expr: string, props: UdbProp[]): string {
  ...
  const referencable = props.filter((p) =>
    !["formula", "rollup", "relation"].includes(p.type)
  );
  ...
  while (i < expr.length) {
    ...
    if (ch === "'") { // string literal ('' escapes)
      let j = i + 1;
      while (j < expr.length && !(expr[j] === "'" && expr[j + 1] !== "'")) {
        j += expr[j] === "'" ? 2 : 1;
      }
    ...
    if (ch === '"') { // quoted property name
      ...
      if (!p) {
        throw new Error(
          `unknown property "${name}" (formulas may reference stored properties only)`,
        );
      }
      out += sub(p);
      i = j + 1;
    ...
    if (m) {
      ...
      if (isFn || SQL_WORDS.has(lower)) out += word;
        ...
        if (!p) {
          throw new Error(
            `unknown identifier "${word}" — not a property or SQL keyword`,
          );
        }
        out += sub(p);
      }
...
export async function getUdb(dbId: string) {
  ...
  const targetDbs = [
    ...new Set(relProps.map((p) => String(p.config.target_db))),
  ];
  ...
  for (const p of relProps) {
    ...
    for (const l of links) {
      (byRow[l.row_id] ??= []).push({ id: l.id, title: l.title, icon: l.icon });
    }
  ...
  return {
    ...
    rows: rows.map((r) => ({
      ...
      relations: Object.fromEntries(
        relProps.map((p) => [p.id, relations[p.id]?.[r.id] ?? []]),
      ),
      derived: Object.fromEntries(
        Object.keys(derived).map((pid) => [pid, derived[pid][r.id] ?? null]),
      ),
    })),
  };
}
