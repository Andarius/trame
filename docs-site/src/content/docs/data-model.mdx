...
| `specs_page_id` | The card's spec page — a lazily created subpage of its story with the full block editor and comments, or an existing page adopted as the specs (**Convert to session** on a page). Note: spec pages follow page ACL (subtree-shareable on the hub), while the session row itself stays member-only. |
