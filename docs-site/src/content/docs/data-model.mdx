...
| `specs_page_id` | The card's spec page — a lazily created subpage of its story with the full block editor and comments. Note: spec pages follow page ACL (subtree-shareable on the hub), while the session row itself stays member-only. |
