...
3. Create a Trame page (see the `trame-page` skill). Pass no parent: the plan files itself under the project owning the working directory. Prefer the `trame_create_page` MCP tool (pass `repo_path`); otherwise pipe JSON to the writer:
...
5. After creating the page, tell the user:
   - Open the page in Trame (give the URL and the project it was filed under, both from the writer output)
