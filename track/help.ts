...
- \`story\` — **Story** the session serves, found-or-created by name under the project; only if evident. A story bound for Cockpit needs a brief on the story page itself: the story files as a user story, and each session tagged for Cockpit (see \`tags\`) files as a ticket under it, with \`next_step\` as the ticket's objective.
